...
var xe = (a, t, e) =>
	t in a ? de(a, t, { enumerable: !0, configurable: !0, writable: !0, value: e }) : (a[t] = e);
var d = (a, t, e) => xe(a, typeof t != "symbol" ? t + "" : t, e),
	be = (a, t, e) => t.has(a) || V("Cannot " + e);
var Y = (a, t, e) =>
	t.has(a)
...
const le = /[&<>"']/,
	me = new RegExp(le.source, "g"),
	oe = /[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,
	we = new RegExp(oe.source, "g"),
	ye = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" },
	ee = (a) => ye[a];
function x(a, t) {
	if (t) {
		if (le.test(a)) return a.replace(me, ee);
	} else if (oe.test(a)) return a.replace(we, ee);
	return a;
}
...
function se(a, t, e, n) {
	...
	if (a[0].charAt(0) !== "!") {
		n.state.inLink = !0;
		const l = { type: "link", raw: e, href: i, title: r, text: s, tokens: n.inlineTokens(s) };
		return (n.state.inLink = !1), l;
	}
	return { type: "image", raw: e, href: i, title: r, text: x(s) };
}
function _e(a, t) {
	const e = a.match(/^(\s+)(?:```)/);
	if (e === null) return t;
...
`,
				...
				this.options.pedantic
					? ((f = 2), (o = u.trimStart()))
					: ((f = e[2].search(/[^ ]/)), (f = f > 4 ? 1 : f), (o = u.slice(f)), (f += e[1].length));
				let y = !1;
				if (
					(!u &&
						/^ *$/.test(h) &&
...
`),
					...
					const M = new RegExp(
							`^ {0,${Math.min(3, f - 1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`
						),
						W = new RegExp(
							`^ {0,${Math.min(3, f - 1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`
						),
						J = new RegExp(`^ {0,${Math.min(3, f - 1)}}(?:\`\`\`|~~~)`),
						K = new RegExp(`^ {0,${Math.min(3, f - 1)}}#`);
					for (; t; ) {
						const Q = t.split(
							`
`,
						...
						if (
							...
							J.test(h) || K.test(h) || M.test(h) || W.test(t))
						)
							break;
...
` + h.slice(f);
							...
							if (y || u.search(/[^ ]/) >= 4 || J.test(u) || K.test(u) || W.test(u)) break;
							o +=
								`
` + h;
						}
...
`)
		...
		if (n.length === i.length) {
			...
			for (const l of r)
				s.rows.push(ne(l, s.header.length).map((o) => ({ text: o, tokens: this.lexer.inline(o) })));
			return s;
		}
	}
	lheading(t) {
...
`
	...
	escape(t) {
		...
		if (e) return { type: "escape", raw: e[0], text: x(e[1]) };
	}
	tag(t) {
	...
	autolink(t) {
		...
		if (e) {
			let n, i;
			return (
				e[2] === "@" ? ((n = x(e[1])), (i = "mailto:" + n)) : ((n = x(e[1])), (i = n)),
	...
	inlineText(t) {
		...
		if (e) {
			let n;
			return (
				this.lexer.state.inRawBlock ? (n = e[0]) : (n = x(e[0])),
...
const Se = /^(?: *(?:\n|$))+/,
	...
	Qe = /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,
	E = "\\p{P}\\p{S}",
	De = k(/^((?![*_])[\spunctuation])/, "u")
		.replace(/punctuation/g, E)
		.getRegex(),
	Oe = /\[[^[\]]*?\]\([^\(\)]*?\)|`[^`]*?`|<[^<>]*?>/g,
	je = k(/^(?:\*+(?:((?!\*)[punct])|[^\s*]))|^_+(?:((?!_)[punct])|([^\s_]))/, "u")
		.replace(/punct/g, E)
		.getRegex(),
	Ne = k(
	...
	ge = k(/^!?\[(ref)\](?:\[\])?/)
		.replace("ref", H)
		.getRegex(),
	Je = k("reflink|nolink(?!\\()", "g").replace("reflink", fe).replace("nolink", ge).getRegex(),
	X = {
		_backpedal: S,
		anyPunctuation: Fe,
		autolink: Ue,
	...
	q = { normal: U, gfm: Be, pedantic: ve },
	_ = { normal: X, gfm: D, breaks: Ve, pedantic: Ke };
class m {
	constructor(t) {
		d(this, "tokens");
		d(this, "options");
		...
		const e = { block: q.normal, inline: _.normal };
		this.options.pedantic
			? ((e.block = q.pedantic), (e.inline = _.pedantic))
			: this.options.gfm &&
				((e.block = q.gfm), this.options.breaks ? (e.inline = _.breaks) : (e.inline = _.gfm)),
			(this.tokenizer.rules = e);
	}
	static get rules() {
		return { block: q, inline: _ };
	}
	static lex(t, e) {
...
`
	...
	blockTokens(t, e = []) {
		this.options.pedantic
			...
			: (t = t.replace(/^( *)(\t+)/gm, (l, o, p) => o + "    ".repeat(p.length)));
		let n, i, r, s;
		for (; t; )
			if (
				!(
...
` + n.raw),
				...
				if (((r = t), this.options.extensions && this.options.extensions.startBlock)) {
					...
					this.options.extensions.startBlock.forEach((c) => {
						(p = c.call({ lexer: this }, o)),
							typeof p == "number" && p >= 0 && (l = Math.min(l, p));
					}),
...
` + n.text),
	...
	inlineTokens(t, e = []) {
		...
		for (; (l = this.tokenizer.rules.inline.anyPunctuation.exec(s)) != null; )
			s =
				s.slice(0, l.index) + "++" + s.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);
		for (; t; )
			if (
				(o || (p = ""),
				(o = !1),
			...
			switch (r.type) {
				...
				case "heading": {
					...
					n += this.renderer.heading(
						...
						s.depth,
						Te(this.parseInline(s.tokens, this.textRenderer))
					);
					continue;
				}
				case "code": {
				...
				case "table": {
					...
					for (let c = 0; c < s.header.length; c++)
						o += this.renderer.tablecell(this.parseInline(s.header[c].tokens), {
							header: !0,
							align: s.align[c],
						});
					...
					for (let c = 0; c < s.rows.length; c++) {
						...
						for (let h = 0; h < u.length; h++)
							o += this.renderer.tablecell(this.parseInline(u[h].tokens), {
								header: !1,
								align: s.align[h],
							});
						...
						if (h.task) {
							...
							p
								? h.tokens.length > 0 && h.tokens[0].type === "paragraph"
									? ((h.tokens[0].text = $ + " " + h.tokens[0].text),
										h.tokens[0].tokens &&
											h.tokens[0].tokens.length > 0 &&
											h.tokens[0].tokens[0].type === "text" &&
											(h.tokens[0].tokens[0].text = $ + " " + h.tokens[0].tokens[0].text))
									: h.tokens.unshift({ type: "text", text: $ + " " })
								: (b += $ + " ");
						}
						(b += this.parse(h.tokens, p)), (c += this.renderer.listitem(b, y, !!f));
					}
...
d(I, "passThroughHooks", new Set(["preprocess", "postprocess", "processAllTokens"]));
var z, O, ke;
class Ye {
	constructor(...t) {
		Y(this, z);
		...
		for (const s of t)
			switch (((n = n.concat(e.call(this, s))), s.type)) {
				case "table": {
					...
					for (const o of l.rows) for (const p of o) n = n.concat(this.walkTokens(p.tokens, e));
					break;
				}
				case "list": {
					const l = s;
				...
				default: {
					const l = s;
					(r = (i = this.defaults.extensions) == null ? void 0 : i.childTokens) != null && r[l.type]
					...
					n.extensions &&
						(n.extensions.forEach((r) => {
							...
							if ("tokenizer" in r) {
								...
								const s = e[r.level];
								s ? s.unshift(r.tokenizer) : (e[r.level] = [r.tokenizer]),
									r.start &&
										(r.level === "block"
											? e.startBlock
		...
		return (n, i) => {
			...
			if ((s.hooks && (s.hooks.options = s), s.async))
				return Promise.resolve(s.hooks ? s.hooks.preprocess(n) : n)
					.then((o) => t(o, s))
					.then((o) => (s.hooks ? s.hooks.processAllTokens(o) : o))
					.then((o) =>
						s.walkTokens ? Promise.all(this.walkTokens(o, s.walkTokens)).then(() => o) : o
					)
					.then((o) => e(o, s))
					.then((o) => (s.hooks ? s.hooks.postprocess(o) : o))
					.catch(l);
			try {
...
g.options = g.setOptions = function (a) {
	return T.setOptions(a), (g.defaults = T.defaults), re(g.defaults), g;
};
g.getDefaults = j;
g.defaults = R;
g.use = function (...a) {
	return T.use(...a), (g.defaults = T.defaults), re(g.defaults), g;
};
